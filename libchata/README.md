...
Ultrassembler ships in the lean and mean `libchata` library and is also available in the `chatacli` tool. Use it anywhere C++20 is supported and link it dynamically or statically. It's your choice.
...
I was originally going to include Ultrassembler as an entirely private part of the `libchata` library that you wouldn't be able to use on its own. However, I realized that with how useful an independent RISC-V assembler might turn out to be, you should be able to use Ultrassembler independently of the rest of the Chata project. That's what you're seeing here.
...
Then, include the line `#include <libchata.hpp>` where you use Ultrassembler.
...
Ultrassembler may throw a `ChataError` exception if it encounters incorrect code or has some other error.
