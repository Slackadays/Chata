...
const uint8_t fast_float_reg_search(const ultrastring& reg) {
    ...
    if (reg[0] == 'f') {
        ...
        if (reg[1] == 's') {
            if (reg.size() < 3) return reg_search_failed;
            if (reg[2] == '0') {
                if (reg.size() < 4) return 8;
            }
            if (reg[2] == '1') {
                if (reg.size() < 4) return 9;
                if (reg[3] == '0') {
                    if (reg.size() < 5) return 26;
                }
                if (reg[3] == '1') {
                    if (reg.size() < 5) return 27;
                }
            }
            if (reg[2] == '2') {
                if (reg.size() < 4) return 18;
        ...
        if (reg[1] == 't') {
            if (reg.size() < 3) return reg_search_failed;
            if (reg[2] == '0') {
                if (reg.size() < 4) return 0;
            }
            if (reg[2] == '1') {
                if (reg.size() < 4) return 1;
                if (reg[3] == '0') {
                    if (reg.size() < 5) return 30;
                }
                if (reg[3] == '1') {
                    if (reg.size() < 5) return 31;
                }
            }
            if (reg[2] == '2') {
                if (reg.size() < 4) return 2;
            }
            if (reg[2] == '3') {
                if (reg.size() < 4) return 3;
            }
            if (reg[2] == '4') {
                if (reg.size() < 4) return 4;
            }
            if (reg[2] == '5') {
                if (reg.size() < 4) return 5;
            }
            if (reg[2] == '6') {
                if (reg.size() < 4) return 6;
            }
            if (reg[2] == '7') {
                if (reg.size() < 4) return 7;
            }
            if (reg[2] == '8') {
                if (reg.size() < 4) return 28;
            }
